...
export const createCurrency = async (req, res) => {
  try {
    ...
    if (!code || !name || !symbol) {
      return res
        .status(400)
        .json({ error: "Code, name, and symbol are required" });
    }
  ...
  try {
    ...
    if (!organizationId || !moduleType || !currencyId) {
      return res
        .status(400)
        .json({
          error: "Organization ID, module type, and currency ID are required",
        });
    }

    const currency = await Currency.setDefault(
      organizationId,
      moduleType,
      currencyId,
    );
...
export const getDefaultCurrency = async (req, res) => {
  try {
    ...
    const currency = await Currency.getDefaultCurrency(
      organizationId,
      moduleType,
    );
...
export const getOrganizationCurrencySettings = async (req, res) => {
  try {
    const { organizationId, moduleType } = req.params;
    const settings = await Currency.getOrganizationCurrencySettings(
      organizationId,
      moduleType,
    );
